...
import time, hashlib, re, os, unicodedata
import requests
...
def _build_session() -> requests.Session:
    retry = Retry(total=3, read=3, connect=3, status=3, backoff_factor=0.5,
                  status_forcelist=(429,500,502,503,504),
                  allowed_methods=frozenset(["GET","HEAD"]), raise_on_status=False)
    s = requests.Session(); s.headers.update(HEADERS)
    ad = HTTPAdapter(max_retries=retry); s.mount("http://", ad); s.mount("https://", ad)
    return s
...
PARSERS = {"vnexpress": extract_article_vnexpress, "thanhnien": extract_article_thanhnien}

# --------- Author sanitize & EIC fallback ----------
...
def _sanitize_author(author: Optional[str], base_url: str) -> Optional[str]:
    ...
    low = a.lower(); host = _urlparse(base_url or "").netloc.lower()
    if EMAIL_RE.search(a): return None
    if low.startswith(("http://","https://")): return None
    if DOMAIN_RE.search(low):
        if (host and host in low) or low == host: return None
        if DOMAIN_RE.fullmatch(low): return None
...
def _map_category_name_from_rss(rss_url: str, site: str = None) -> str:
...
def _upsert_article_from_extracted(db, src: dict, ex_doc: dict, allow_update: bool = True, verbose_log=None):
    ...
    if not (site and external_url and title and content):
        if verbose_log: verbose_log(f"[SKIP] inline-import thiếu field url={external_url}")
        return {"skipped":1}
    ...
    # TN: nếu section null/trùng cha → đoán từ url (chỉ để hiển thị), nhưng vẫn lookup-only khi gán ID
    ...
    if section and not _is_same_section_as_parent(section, parent_name):
        ...
        # Không tìm thấy -> None (KHÔNG tạo mới)
    ...
    set_doc = {
        ...
        "category_id": parent_id, "category_child_id": category_child_id,
        "published_at": published_at, "images": images, "updated_at": now_vn()
    }
    ...
    if res.matched_count == 0 and res.upserted_id is not None:
        if verbose_log: verbose_log(f"[OK]   INLINE INSERT | {title} | child={section or '-'}")
        return {"inserted":1}
    elif allow_update and res.modified_count > 0:
        if verbose_log: verbose_log(f"[OK]   INLINE UPDATE | {title} | child={section or '-'}")
        return {"updated":1}
    else:
        if verbose_log: verbose_log(f"[SKIP] INLINE EXIST  | {title}")
        return {"skipped":1}

def crawl_rss_source(source: Dict[str, Any], limit: int = 50, skip_existing: bool = True,
                     log: Optional[Callable[[str], None]] = None) -> Dict[str, Any]:
    db = get_db()
    crawl_runs = db.crawl_runs; raw_pages = db.raw_pages; extracted = db.extracted_articles

    source_id = source.get("_id"); name_source = source.get("name_source")
    ...
    run_id = crawl_runs.insert_one({
        "source_id": source_id, "started_at": now_vn(), "finished_at": None,
        "status": "running", "stats": {"pages":0,"saved":0,"skipped":0,"errors":0}, "stats_debug":[]
    }).inserted_id

    session = _build_session(); fetched=saved=skipped=0
    ...
    try:
        ...
        for e in entries:
            ...
            if u and isinstance(u, str) and u.startswith("http"): feed_urls.append(u)
        ...
        for idx, e in enumerate(entries, start=1):
            if os.path.exists("STOP_RSS.txt"):
                if log: log("[STOP] Phát hiện STOP_RSS.txt — dừng crawl.")
                break

            url = getattr(e, "link", None)
            if not url or not url.startswith("http"):
                if log: log(f"[SKIP] #{idx:02d} URL không hợp lệ"); continue
            ...
            try:
                ...
                status = resp.status_code; ctype = resp.headers.get("Content-Type"); html = None
                if status == 200:
                ...
                raw_doc = {
                    ...
                    "crawler_time": now_vn(), "http_status": int(status) if status is not None else None,
                ...
                if status == 200 and html:
                    ...
                    if getattr(e, "published_parsed", None):
                        tm = e.published_parsed; published_at = datetime(*tm[:6], tzinfo=ZoneInfo("Asia/Ho_Chi_Minh"))

                    title=body_text=None; images: List[str]=[]; author: Optional[str]=None; section=None

                    if parse_func:
                        art = parse_func(html, base_url)
                        title = art.get("title"); body_text = art.get("body")
                    ...
                    # TN: nếu section null/trùng cha → đoán từ url để hiển thị (ID vẫn lookup-only)
                    parent_name = _map_category_name_from_rss(rss_url, site)
                    ...
                    if not (title and body_text):
                        db.crawl_runs.update_one({"_id": run_id}, {"$push": {"stats_debug": {"url": url, "reason": "no_title_or_body"}}})
                        ...
                        try:
                            db.extracted_articles.insert_one(ex_doc)
                            saved += 1
                            if log: log(f"[OK]   #{idx:02d} EXTRACTED | title='{title}' | child='{section or '-'}' | imgs={len(images)} | author='{author or 'NULL'}'")
                            # ===== Inline import -> lookup-only category_child =====
                            try:
                ...
                db.crawl_runs.update_one({"_id": run_id},
                    {"$set": {"stats.pages": fetched, "stats.saved": saved, "stats.skipped": skipped}})

            except Exception as ex:
                db.crawl_runs.update_one({"_id": run_id}, {"$push": {"stats_debug": {"url": url, "reason": "exception", "msg": str(ex)}}})
                if log: log(f"[ERROR]#{idx:02d} Exception: {ex}")

        db.crawl_runs.update_one({"_id": run_id}, {"$set": {
            "status":"success","finished_at":now_vn(),
